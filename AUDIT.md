...
1. **Directory prefix false positives** — `pathPrefix` uses raw `startsWith`, so `/projects/app` also matches `/projects/app-v2` (`src/core/matcher.ts:23-26`). Reproduced locally.
2. **Windows separator mismatch** — normalization strips only trailing separators; `C:\\work\\app` does not match a `C:/work` rule (`src/core/types.ts:106-109`, `src/core/matcher.ts:24-26`). Reproduced locally.
3. **Hidden rule shadows later active rules** — classification selects the first rule before checking whether it is hidden, forcing an otherwise valid later match to top level (`src/core/matcher.ts:35-40,129-141`).
...
6. **Dropping a workspace on itself moves it to the end** — `moveBefore`/`moveAfter` remove the item before looking up the same target (`src/core/matcher.ts:190-207`). Reproduced locally.
...
2. **Reserved `__topLevel__` name is accepted** — a manual group can collide with the internal top-level order key (`src/core/types.ts:98-104`; `src/host-manual.ts:43-63`; `src/client/GroupsBrowser.tsx:418-428`).
3. **Assignments to hidden categories can disappear from both group and top-level trees** — direct manual overrides are returned without checking effective-category availability (`src/core/matcher.ts:129-141`; `src/client/tree.ts:166-170,239-241`).
...
pnpm typecheck                 PASS
pnpm test                      PASS — 4 files, 68 tests
