...
临时项目随手建个「临时」分组拖进去，用完删除分组即全部回到「未分类」。
...
- **从分组拖出**：即使所有项目都在分组里（未分类桶为空），**拖拽中未分类桶也会
  临时显示**，随时可把项目拖出分组；规则分类里的项目行菜单同样有「移到未分类」
...
- v0.4.1 真机验证 34/34（新增：**从分组拖出**——未分类桶为空时拖拽中也显示，
  拖到其上 = 强制未分类；规则分类项目行也有「移到未分类」菜单）。
- 单测 68 用例全绿（vitest：`core` / `manual` / `tree` / `store`）。
