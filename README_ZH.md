...
- **会话行限制与置顶会话**：项目展开后默认显示五行（必要时另加当前会话、置顶会话及带颜色标记的会话），并提供临时的
  **展开全部 / 折叠** 控件。支持在会话的 `⋯` 菜单中将特定会话在工作区内置顶或取消置顶（**置顶会话 / 取消置顶**）；
  置顶会话显示图钉图标，并始终固定在工作区顶部且不被分页折叠隐藏。
- **分组、项目与会话颜色标记**：悬停行后使用颜色按钮，从八种预设中标记该行。搜索结果使用同一套紧凑 portal 菜单。带颜色标记的会话会保留在五行预览中；Finder 颜色筛选会匹配该颜色的分组、项目，或任意被标记的会话。

### 分组管理（完整生命周期）
- **手动新建分组**：区头「新建分组」按钮即建即显，空分组也渲染
- **直接在分组内添加工作区**：在任意分组行上点击快捷「在此分组添加工作区」按钮，
  或在其 `⋯` 菜单中选择「添加工作区到分组」，即可选择/创建目录并直接将其归入该分组中，
  自动更新并在持久化 overlay 中生效，无需新建后再手动拖拽
...
- **Finder 风格筛选**：状态范围可切换 **全部 / 需要处理 / 运行中 / 新结果**；**需要处理**
  同时包含 Awaiting 和 Error 会话。筛选菜单可再选择一种分组/项目/会话颜色，以及最近 24 小时、
...
- **固定筛选栏与 Chip**：状态范围栏、筛选菜单与已启用条件 Chip 在视图区域上方固定放置，
  树视图滚动时保持置顶。
- **会话清理与归档**：批量归档超过设定天数（7、14、30、60、90 天或自定义天数）未活动的旧会话。支持从树层操作全局菜单（⋯）或工作区右键菜单针对单个项目执行。自动排除正在运行的 Agent、等待交互以及当前活动会话。
- **工作区/会话操作不退化**：Add Workspace、项目重命名/删除、新建/打开/重命名/
  派生/归档会话以及会话置顶/取消置顶。

### 持久化与零侵入
- 所有手动操作（分组、归类、排序、改名、隐藏、置顶会话、颜色标记）写入插件自有 overlay
...
- `colors` —— 可选颜色标记，键为分组名、工作区 id 或会话 id（八种预设之一）。
- `pinnedSessions` —— 按工作区记录的置顶会话 id 列表，这些会话固定显示在该项目顶部。
...
| 设置分组/项目/会话颜色 | 悬停行后使用颜色按钮；紧凑 portal 菜单始终限制在视口内 |
