...
- [ ] Исправить нормализацию путей для `/` и `\\`, корневых путей и Windows drive letter.
- [ ] Сделать `pathPrefix` сравнением по границе сегмента, а не строковым `startsWith`.
- [ ] Пропускать скрытые правила и корректно обрабатывать назначения в скрытые/удалённые группы.
- [ ] Зарезервировать `__topLevel__` на Host и Client boundaries.
- [ ] Сделать self-drop гарантированным no-op.
