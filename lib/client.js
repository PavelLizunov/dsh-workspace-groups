	...
	factory: (require) => {
		...
		*/
		function deriveTopLevel(list, workspaces, archivedSessionIds, config, view, manual) {
			...
			const nodes = [];
			for (const workspace of workspaces) {
				if (resolveCategory(config, manual, workspace.workspaceId, workspace.path, workspace.title) !== void 0) continue;
				const sessions = workspaceSessions(list, workspace, archived, descendants);
				const wsExpanded = expandedWorkspaces.has(workspace.workspaceId);
				nodes.push({
					workspaceId: workspace.workspaceId,
		...
		function GroupsBrowser({ wide, expandSidebar, useSessions, useWorkspaces, useStore, actions, startSession, open, renameSession, forkSession, renameWorkspace, deleteWorkspace, insertWorkspaceBefore, archiveSession, insertSessionBefore, createWorkspace, pickDirectory, searchSessions, searchResultLimit, t }) {
			...
			*/
			...
			const onDragOverRow = (row) => (event) => {
				...
				if (row.kind === "topLevel") {
					setDragIndicator((prev) => prev?.mode === "into" && prev.categoryKey === UNCATEGORIZED_KEY ? prev : {
						mode: "into",
		...
		*/
		function TopLevelSection({ topLevel, current, now, t, showDropTarget, areaActive, dropActive, onDragOverRow, onDragLeaveRow, onDropRow, onDragStartWorkspace, onToggleWorkspace, onNewSession, onOpen, onRenameRequest, onDeleteRequest, onSessionRename, onSessionArchive, onFork }) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [showDropTarget && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
					className: `wgTopLevelDrop${dropActive ? " wgDropTarget" : ""}`,
					role: "treeitem",
					onDragOver: onDragOverRow(topRef),
					onDragLeave: onDragLeaveRow,
					onDrop: onDropRow(UNCATEGORIZED_KEY, topRef),
					children: t("group.dropTopLevel")
