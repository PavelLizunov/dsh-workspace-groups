window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		function deriveSearchGroups(list, workspaces, config, matchedIds, archivedSessionIds, manual, snippetsBySession, pendingInteractions = /* @__PURE__ */ new Map()) {
			...
			for (const workspace of workspaces) {
				...
				for (const id of workspace.sessionIds) {
					...
					const node = sessionNode(summary, descendants, pinnedSet.has(id), pendingInteractions, manual.colors?.[id]);
		...
		function sessionNode(s, descendants, pinned, pendingInteractions = /* @__PURE__ */ new Map(), color) {
		...
		function workspaceSessions(list, workspace, archived, descendants, pinnedIds, onSession, pendingInteractions = /* @__PURE__ */ new Map(), colors) {
			const pinnedSet = new Set(pinnedIds ?? []);
		...
		function deriveWorkspaceTree(list, workspaces, archivedSessionIds, config, manual, pendingInteractions = /* @__PURE__ */ new Map()) {
			const archived = new Set(archivedSessionIds);
			const descendants = indexSubagentDescendants(list.byId);
		...
		*/
		function GroupsBrowser({ wide, expandSidebar, useSessions, useSessionPendingInteraction, useWorkspaces, useStore, actions, startSession, open, renameSession, forkSession, renameWorkspace, deleteWorkspace, insertWorkspaceBefore, archiveSession, cleanupSessions, insertSessionBefore, createWorkspace, listDirectory, createDirectory, searchSessions, searchResultLimit, t }) {
			...
			const canonicalTree = (0, react.useMemo)(() => normalizedQuery === "" ? deriveWorkspaceTree(list, workspaces, archivedSessionIds, config, manual, pendingInteractions) : EMPTY_WORKSPACE_TREE, [
				list,
				workspaces,
				archivedSessionIds,
				config,
