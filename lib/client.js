window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		function deriveGroups(list, workspaces, archivedSessionIds, config, view, manual) {
			...
			for (const workspace of workspaces) {
				const key = resolveCategory(config, manual, workspace.workspaceId, workspace.path, workspace.title) ?? UNCATEGORIZED_KEY;
			...
			const nodes = [];
			const orderedKeys = [...effectiveCategories(config, manual).map((e) => e.key), UNCATEGORIZED_KEY];
		...
		*/
		function withDraggingUncategorized(groups, dragging) {
		...
		*/
		...
		function WorkspaceRow({ node, t, onToggle, onNewSession, onRename, onDelete, canMoveOut = false, onMoveOut, dropActive = false, insertLine, onRowDragOver, onRowDragLeave, onRowDrop, onDragStartExtra }) {
			const [menuOpen, setMenuOpen] = (0, react.useState)(false);
			const menuItems = [
				...canMoveOut && onMoveOut !== void 0 ? [{
					id: "moveOut",
					label: t("workspace.moveToUncategorized"),
		...
		*/
		function GroupsBrowser({ wide, expandSidebar, useSessions, useWorkspaces, useStore, actions, startSession, open, renameSession, forkSession, renameWorkspace, deleteWorkspace, insertWorkspaceBefore, archiveSession, insertSessionBefore, createWorkspace, pickDirectory, searchSessions, searchResultLimit, t }) {
			...
			const displayGroups = (0, react.useMemo)(() => withDraggingUncategorized(groups, dragging), [groups, dragging]);
			...
			const moveWorkspaceTo = async (workspaceId, categoryKey, beforeWorkspaceId, afterWorkspaceId) => {
				...
				try {
					...
					if (categoryKey === UNCATEGORIZED_KEY) {
						...
						const workspaceOrder = Object.fromEntries(Object.entries(manual.workspaceOrder).map(([key, ids]) => [key, ids.filter((id) => id !== workspaceId)]));
						next = {
							...manual,
							assignments,
							workspaceOrder
			...
			*/
			...
			const onDragOverRow = (row) => (event) => {
				...
				const rect = event.currentTarget.getBoundingClientRect();
				const before = event.clientY < rect.top + rect.height / 2;
				if (draggingCategory || row.kind === "workspace") setDragIndicator((prev) => prev?.mode === "line" && prev.row.kind === row.kind && prev.row.key === row.key && prev.before === before ? prev : {
					mode: "line",
					row,
					before
				});
			...
			const onDragStartWorkspace = (workspaceId) => () => {
				setDragging(true);
				const workspace = workspaces.find((w) => w.workspaceId === workspaceId);
				const sourceKey = workspace === void 0 ? void 0 : resolveCategory(config, manual, workspace.workspaceId, workspace.path, workspace.title);
				for (const key of Object.keys(categoryExpansion)) {
					if (!categoryExpansion[key]) continue;
					if (sourceKey !== void 0 && key === sourceKey) continue;
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							wide && normalizedQuery !== "" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(SearchBody, {
								...
								}), displayGroups.map((category) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)(CategorySection, {
									category,
									current,
									now,
									t,
		...
		*/
		function SearchBody({ list, workspaces, config, archivedSessionIds, query, remote, resultLimit, current, now, open, manual, t }) {
			...
			const groups = (0, react.useMemo)(() => deriveSearchGroups(list, workspaces, config, matches.matchedIds, archivedSessionIds, manual, matches.snippetsBySession), [
				list,
				workspaces,
				config,
				matches,
