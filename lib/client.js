window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		function workspaceSessions(list, workspace, archived, descendants, onSession) {
			const nodes = [];
		...
		*/
		...
		function applySidebarFilter(categories, topLevel, filter, colors, now) {
			...
			function filterWorkspace(workspace, categoryKey) {
				if (!isWorkspaceColorMatched(workspace.workspaceId, categoryKey, filter.color, colors)) return null;
		...
		*/
		...
		function SessionRow({ node, currentId, now, t, onOpen, onRename, onFork, onArchive, actionBusy = false, "aria-level": ariaLevel, "aria-posinset": ariaPosinset, "aria-setsize": ariaSetsize }) {
			...
			const ariaLabel = pillLabel !== void 0 ? `${node.title} (${pillLabel})` : node.title;
			const menuItems = [
				...onRename !== void 0 ? [{
					id: "rename",
