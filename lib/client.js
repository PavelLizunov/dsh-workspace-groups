window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function DirectoryBrowser({ open, busy, listDirectory, createDirectory, onPick, onClose, strings }) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.Modal, {
				...
				footer: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.Button, {
					...
					variant: "primary",
					disabled: targetPath === void 0 || loading || busy || creating,
					onClick: () => {
				...
				children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							...
							children: (listing?.crumbs ?? []).map((crumb, index) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
								...
								children: [index > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.IconChevronRightOutline14, { size: 12 }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
									...
									children: index === 0 ? strings.home : crumb.name
								})]
							}, crumb.path))
						}),
						...
						listing !== null && folderDraft === null && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
							type: "button",
		...
		*/
		function CategoryRow({ node, t, onToggle, onRename, onDelete, dropActive = false, insertLine, onRowDragOver, onRowDragLeave, onRowDrop, onDragStartCategory }) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					(manageable || onDragStartCategory !== void 0) && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
						className: "wgRowActions",
						children: [onDragStartCategory !== void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
							type: "button",
		...
		function WorkspaceRow({ node, t, onToggle, onNewSession, onRename, onDelete, canMoveOut = false, onMoveOut, moveTargets, onMoveTo, flat = false, dropActive = false, insertLine, onRowDragOver, onRowDragLeave, onRowDrop, onDragStartExtra }) {
			const [menuOpen, setMenuOpen] = (0, react.useState)(false);
			const menuItems = [
		...
		function GroupsBrowser({ wide, expandSidebar, useSessions, useWorkspaces, useStore, actions, startSession, open, renameSession, forkSession, renameWorkspace, deleteWorkspace, insertWorkspaceBefore, archiveSession, insertSessionBefore, createWorkspace, listDirectory, createDirectory, searchSessions, searchResultLimit, t }) {
			...
			(0, react.useEffect)(() => {
				const clear = () => {
					...
					liveExpansionRef.current.actions.restoreExpansionSnapshot(snapshot.original, [...snapshot.touchedCategories], [...snapshot.touchedWorkspaces]);
				};
			...
			const moveWorkspaceTo = async (workspaceId, categoryKey, beforeWorkspaceId, afterWorkspaceId) => {
				...
				if (currentKey === (categoryKey === UNCATEGORIZED_KEY ? void 0 : categoryKey) && beforeWorkspaceId === void 0 && afterWorkspaceId === void 0) return;
				...
				try {
					...
					if (categoryKey === UNCATEGORIZED_KEY) {
						...
						const topLevelMembers = workspaces.filter((w) => w.workspaceId === workspaceId || resolveCategory(config, manual, w.workspaceId, w.path, w.title) === void 0).map((w) => w.workspaceId);
						const topLevelOrder = orderedWorkspaceIds(manual, TOP_LEVEL_ORDER_KEY, topLevelMembers);
						workspaceOrder[TOP_LEVEL_ORDER_KEY] = afterWorkspaceId !== void 0 ? moveAfter(topLevelOrder, workspaceId, afterWorkspaceId) : moveBefore(topLevelOrder, workspaceId, beforeWorkspaceId);
						next = {
							...manual,
							assignments,
							workspaceOrder
						...
						const targetMembers = workspaces.filter((w) => w.workspaceId === workspaceId || resolveCategory(config, manual, w.workspaceId, w.path, w.title) === categoryKey).map((w) => w.workspaceId);
						const effectiveTargetOrder = orderedWorkspaceIds(manual, categoryKey, targetMembers);
						const targetOrder = afterWorkspaceId !== void 0 ? moveAfter(effectiveTargetOrder, workspaceId, afterWorkspaceId) : moveBefore(effectiveTargetOrder, workspaceId, beforeWorkspaceId);
						const workspaceOrder = {
			...
			*/
			...
			const onDragStartWorkspace = (_workspaceId) => () => {
				...
				const temporaryWorkspaces = Object.fromEntries(Object.entries(workspaceExpansion).map(([key, value]) => [key, value ? false : value]));
				expansionSnapshot.current = {
					original: {
						categories: { ...categoryExpansion },
						workspaces: { ...workspaceExpansion }
					},
					touchedCategories: /* @__PURE__ */ new Set(),
					touchedWorkspaces: /* @__PURE__ */ new Set()
				};
				for (const [key, value] of Object.entries(temporaryWorkspaces)) if (workspaceExpansion[key] !== value) actions.setWorkspaceExpanded(key, value);
			};
			const onDragStartCategory = (categoryKey) => (event) => {
				event.dataTransfer.setData(DND_CATEGORY_TYPE, categoryKey);
				event.dataTransfer.effectAllowed = "move";
				setDragging("category");
				const temporaryCategories = Object.fromEntries(Object.entries(categoryExpansion).map(([key, value]) => [key, value ? false : value]));
				expansionSnapshot.current = {
					original: {
						categories: { ...categoryExpansion },
						workspaces: { ...workspaceExpansion }
				...
				for (const [key, value] of Object.entries(temporaryCategories)) if (categoryExpansion[key] !== value) actions.setCategoryExpanded(key, value);
			};
			const now = Date.now();
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
