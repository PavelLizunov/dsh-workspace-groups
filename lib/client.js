window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function deriveSearchGroups(list, workspaces, config, matchedIds, archivedSessionIds, manual, snippetsBySession) {
			...
			for (const workspace of workspaces) {
				...
				for (const id of workspace.sessionIds) {
					...
					const node = sessionNode(summary, descendants, pinnedSet.has(id), manual.colors?.[id]);
		...
		*/
		...
		function sessionNode(s, descendants, pinned, color) {
		...
		function workspaceSessions(list, workspace, archived, descendants, pinnedIds, onSession, colors) {
			const pinnedSet = new Set(pinnedIds ?? []);
		...
		function deriveWorkspaceTree(list, workspaces, archivedSessionIds, config, manual) {
			const archived = new Set(archivedSessionIds);
			const descendants = (0, _deepseek_ai_dsh_client_runtime_client.indexSubagentDescendants)(list.byId);
		...
		function applySidebarFilter(categories, topLevel, filter, colors, now) {
			...
			function filterWorkspace(workspace, categoryKey) {
				...
				for (const session of workspace.sessions) {
					...
					if (!(filter.status === "all" || (filter.status === "warning" ? state === "warning" || state === "error" : state === filter.status))) continue;
		...
		*/
		function findOldSessionsToArchive(sessions, options) {
			...
			return sessions.filter((session) => {
				...
				if (session.pendingInteraction !== void 0) return false;
		...
		*/
		...
		/**
		* Color palette menu for categories and workspaces.
		*/
		...
		*/
		function CategoryRow({ node, t, onToggle, onExpandEntire, onCollapseEntire, onAddWorkspace, onRename, onDelete, color, onSetColor, dropActive = false, insertLine, onRowDragOver, onRowDragLeave, onRowDrop, onDragStartCategory, onMoveUp, onMoveDown, isFirst, isLast, canMoveUp, canMoveDown, "aria-level": ariaLevel, "aria-posinset": ariaPosinset, "aria-setsize": ariaSetsize }) {
			const [menuOpen, setMenuOpen] = (0, react.useState)(false);
			const count = node.workspaces.length;
			const manageable = onRename !== void 0 && onDelete !== void 0 || onExpandEntire !== void 0 || onCollapseEntire !== void 0 || onAddWorkspace !== void 0;
		...
		function WorkspaceRow({ node, t, onToggle, onNewSession, onRename, onDelete, onCleanup, color, onSetColor, canMoveOut = false, onMoveOut, moveTargets, onMoveTo, onMoveUp, onMoveDown, onOpenFolder, onCopyPath, isFirst, isLast, canMoveUp, canMoveDown, flat = false, draggable = false, dropActive = false, insertLine, onRowDragOver, onRowDragLeave, onRowDrop, onWorkspaceDragStart, "aria-level": ariaLevel, "aria-posinset": ariaPosinset, "aria-setsize": ariaSetsize }) {
			const [menuOpen, setMenuOpen] = (0, react.useState)(false);
			const menuItems = (0, react.useMemo)(() => [
				...onMoveUp !== void 0 ? [{
					id: "moveUp",
		...
		*/
		function GroupsBrowser({ wide, expandSidebar, useSessions, useWorkspaces, useStore, actions, startSession, open, renameSession, forkSession, renameWorkspace, deleteWorkspace, insertWorkspaceBefore, archiveSession, insertSessionBefore, createWorkspace, listDirectory, createDirectory, searchSessions, searchResultLimit, t }) {
			...
			const canonicalTree = (0, react.useMemo)(() => normalizedQuery === "" ? deriveWorkspaceTree(list, workspaces, archivedSessionIds, config, manual) : EMPTY_WORKSPACE_TREE, [
				list,
				workspaces,
				archivedSessionIds,
				config,
			...
			const filterResult = (0, react.useMemo)(() => isFilterActive ? applySidebarFilter(canonicalTree.categories, canonicalTree.topLevel, filter, manual.colors, Date.now()) : canonicalTree, [
				canonicalTree,
				filter,
				isFilterActive,
				manual.colors
			...
			const onCleanupConfirm = async () => {
				...
				try {
					for (const session of cleanupCandidates) await archiveSession(session.id);
		...
		*/
		function SearchBody({ list, workspaces, config, archivedSessionIds, query, remote, resultLimit, current, now, open, manual, t, startSession, filter, onCountsChange, onResetFilter, onWorkspaceRename, onWorkspaceDelete, onWorkspaceCleanup, onSessionRename, onSessionFork, onSessionArchive, onSessionPinToggle, sessionActionBusy, onSetItemColor }) {
			...
			const filteredSearch = (0, react.useMemo)(() => applySidebarFilter(searchTree.categories, searchTree.topLevel, filter, manual.colors, Date.now()), [
				searchTree,
				filter,
				manual.colors
			]);
